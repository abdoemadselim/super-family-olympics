    ...
    <div className="space-y-6 text-center" dir="rtl">
      <div className="space-y-3">
        ...
        <h2
          ...
          style={{ color: "hsl(240 20% 15%)", fontFamily: "Cairo, sans-serif" }}
        >
          بطل مجال {category?.icon} {category?.name}
        </h2>
        <div
          className="rounded-3xl p-5 shadow-md"
          style={{ background: "linear-gradient(135deg, hsl(35 95% 55%), hsl(262 83% 58%))" }}
        >
          <p className="text-white font-black text-2xl" style={{ fontFamily: "Cairo, sans-serif" }}>
            {winner?.childName} & {winner?.adultName}
          </p>
          <p className="mt-1" style={{ color: "rgba(255,255,255,0.8)", fontFamily: "Cairo, sans-serif" }}>
      ...
      <div className="space-y-2">
        <h3 className="font-semibold" style={{ color: "hsl(240 10% 50%)", fontFamily: "Cairo, sans-serif" }}>
          نتائج المجال
        </h3>
        {sorted.map((team, i) => (
          <div
            key={team.id}
            className="bg-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-sm border border-amber-100"
          >
            <span className="text-xl">{["🥇", "🥈", "🥉"][i] || "🎖️"}</span>
            <div className="flex-1 text-right">
              <p className="font-bold" style={{ color: "hsl(240 20% 15%)", fontFamily: "Cairo, sans-serif" }}>
      ...
      <button
        onClick={backToMenu}
        className="w-full text-white font-bold py-4 rounded-2xl shadow-md text-lg hover:opacity-90 active:scale-95 transition-all"
        style={{
